...
set JAVA_HOME=D:\ICP_CI_Master\tools\jdk
set ECLIPSE_CLEAN=D:\Opsdev_Project\eclipse-SDK-3.7.1-win32\eclipse
...
set DEPLOY_DIR=%BUILDDIR%..\..\..\deploy
...
cd /d %BUILDDIR%
ant -logfile ./opsbuild.log -DbuildDirectory=%BUILD_DIR%\Pydev -Ddeploy.dir=%DEPLOY_DIR%\Pydev -Dvanilla.eclipse=%ECLIPSE_CLEAN%  -DKEYSTORE=%KEYSTORE% -DSTOREPASS=%STOREPASS%  -Dlocal_build=true -Dlauncher.plugin=%LAUNCHER_PLUGIN% -Dbuilder.plugin=%BUILDER_PLUGIN% -Dbaseos=%BASEOS% -Dbasews=%BASEWS% -Dbasearch=%BASEARCH% -DKEYSTORE_ALIAS=%KEYSTORE_ALIAS% 
