...
public class AddPostBodyPage2 extends WizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        btnJsonForUrl = new Button(container, SWT.RADIO);
        btnJsonForUrl.setText("url");
        btnJsonForUrl.addSelectionListener(new SelectionListener() {

            public void widgetSelected(SelectionEvent e) {
                if (btnJsonForUrl.getSelection()) {
    ...
    public void createItem(UrlBean currentUrlBean) {
    ...
    private void changePreview() {
        ...
        if (isCurrentChosedBeandsNotNull()) {
            if (jsonRadioButton.getSelection() && currentChosedBeands.size() > 0) {
                createdBody = ApiJsonXmlPreviewManager.showJsonForUrl(currentChosedBeands, varianText.getText());
                for (String str1 : createdBody) {
                    str.append(str1 + "\n");
                }
            } else if (setMethodRadioButton.getSelection() && currentChosedBeands.size() > 0) {
                createdBody = ApiJsonXmlPreviewManager.showSetMethon(currentChosedBeands, varianText.getText());
                for (String str1 : createdBody) {
                    str.append(str1 + "\n");
                }
            } else if (btnJsonForUrl.getSelection() && currentChosedBeands.size() > 0) {
                createdBody = ApiJsonXmlPreviewManager.showJsonWithOutUrl(currentChosedBeands, varianText.getText());
                for (String str1 : createdBody) {
                    str.append(str1 + "\n");
                }
            }
